...
# Usage in your MLP class:
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features, out_features, dropout=0.1):
        super().__init__()
        self.fc1 = nn.Linear(in_features, hidden_features)
        self.act = FHNActivation()  # Use FHN activation
        self.fc2 = nn.Linear(hidden_features, out_features)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        x = self.fc1(x)
        x = self.act(x)  # Apply FHN activation
        x = self.dropout(x)
        x = self.fc2(x)
        x = self.dropout(x)
        return x
...
model = train_model(model, train_dataloader, val_dataloader, criterion, optimizer, num_epochs, 'vit_with_FHN_v1.txt')

# Evaluate the model
print("Evaluating model...")
test_acc = evaluate_model(model, test_dataloader, 'vit_with_FHN_v1.txt')
