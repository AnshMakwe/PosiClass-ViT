...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=512, patch_size=16, in_chans=3, num_classes=4,
                 ...
                 qkv_bias=True, drop_rate=0.1, attn_drop_rate=0.1, margin=0.1):
        super().__init__()
        ...
        # self.ce_weight = 1.0  # Primary driver for classification
        # self.ordinal_weight = 0.3  # Regularization term
        ...
        self.class_pos = nn.Parameter(torch.randn(num_classes, embed_dim))

        # Ordinal loss parameters
        self.margin = margin
        self.ce_weight = 1
        self.ordinal_weight = 0.3
    ...
    def forward(self, x):
        ...
        logits = x @ self.class_pos.T  # (B, num_classes)
        return logits
    
    def ordinal_regularization(self):
        loss = 0
        for k in range(self.num_classes - 2):
            c_k = self.class_pos[k]
            c_k1 = self.class_pos[k+1]
            c_k2 = self.class_pos[k+2]
            
            dist1 = torch.norm(c_k1 - c_k, p=2)
            dist2 = torch.norm(c_k2 - c_k, p=2)
            
            violation = (dist1 + self.margin) - dist2
...
def train_one_epoch(model, dataloader, criterion, optimizer, device):
    ...
    for inputs, labels in dataloader:
        ...
        ordinal_loss = model.ordinal_regularization()
        loss = model.ce_weight * ce_loss + model.ordinal_weight * ordinal_loss
...
def train_model(model, train_loader, val_loader, test_loader, criterion, optimizer, scheduler, num_epochs, device, log_file_path):
    ...
    with open(log_file_path, 'a') as log_file:
        ...
        for epoch in range(num_epochs):
            ...
            if val_acc > best_val_acc:
                ...
                torch.save(model.state_dict(), 'best_model_128_4_class_positional_emb.pth')
    ...
    model.load_state_dict(torch.load('best_model_128_4_class_positional_emb.pth'))
...
history = train_model(
    ...
    num_epochs, device, "vit_vanilla_128_4_class_positional_emb.txt"
)
